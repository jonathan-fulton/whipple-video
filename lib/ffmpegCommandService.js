...
class FfmpegCommandService {
    ...
    createFfmpegCommand(command, allowUnknown = false) {
        ...
        this._validateCommandSchema(commandInArrayFormat, schema, allowUnknown);
    ...
    _createCommandSchema(outputRequired = true) {
        ...
        const whippleCommandVideoSchema = Joi.array().items(Joi.object().keys({
        ...
        const whippleCommandAudioSchema = Joi.array().items(Joi.object().keys({
            filePath: Joi.string().required(),
            trimStart: Joi.number().min(0).max(600),
            trimDuration: Joi.number().min(0).max(600)
        }).required());
        ...
        const whippleCommandBackgroundOverlaySchema = Joi.array().items(Joi.object().keys({
        ...
        const whippleCommandTextOverlaySchema = Joi.array().items(Joi.object().keys({
        ...
        const whippleCommandImageOverlaySchema = Joi.array().items(Joi.object().keys({
            filePath: Joi.string().required(),
