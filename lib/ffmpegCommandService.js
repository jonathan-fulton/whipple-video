...
class FfmpegCommandService {
    ...
    _buildFilterComplex(command) {
        ...
        command.video.forEach((video,i) => {
            filter += ` [${i}:v] scale=${command.output.dimensions.width}:${command.output.dimensions.height}:force_original_aspect_ratio=decrease, setdar=${command.output.dimensions.width/command.output.dimensions.height}, trim=start=${video.trimStart || 0}:duration=${video.trimDuration || 600}, setpts=PTS-STARTPTS [v${i}];`
        });
    ...
    _createFilterForTextOverlay(textOverlay, i, command) {
        ...
        // scale the margin with the same ratio as font size
        let defaultMargin = this._fontConfig.defaultMargin || DEFAULT_FONT_MARGIN;
        defaultMargin = (16 / defaultMargin) * (command.output.dimensions.height / 16);

        const x = this._calculateHorizontalPosition(textOverlay.xLoc, defaultMargin, command.output.dimensions);
        const y = this._calculateVerticalPosition(textOverlay.yLoc, defaultMargin, command.output.dimensions);
        const size = (16 / textOverlay.fontSize) * (command.output.dimensions.height / 16);
    ...
    _calculateHorizontalPosition(x, defaultMargin, dimensions) {
        switch (x) {
            case 'left':
            ...
            case 'right':
                return `main_w - text_w - (${defaultMargin} * main_w / ${dimensions.width})`;

            default:
                return `${x} * main_w / ${dimensions.width}`;
        }
    }

    _calculateVerticalPosition(y, defaultMargin, dimensions) {
        switch (y) {
            case 'top':
                return `${defaultMargin}`;
            ...
            case 'bottom':
                return `main_h - text_h - (${defaultMargin} * main_h / ${dimensions.height})`;

            default:
                return `${y} * main_h / ${dimensions.height}`;
        }
    }
