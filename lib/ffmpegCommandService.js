...
class FfmpegCommandService {
    ...
    _buildFilterComplex(command) {
        ...
        command.video.forEach((video,i) => {
            filter += ` [${i}:v]`; // start
            filter += ` pad=ih*${ratio}/sar:ih:(ow-iw)/2:(oh-ih)/2,`; // Force padding for videos not in the correct aspect ratio
            filter += ` scale=${outWidth}:${outHeight}:force_original_aspect_ratio=decrease, setsar=1,`; // Scale videos to the same size (setsar=1 recommended)
            filter += ` trim=start=${video.trimStart || 0}:duration=${video.trimDuration || 600}, setpts=PTS-STARTPTS`; // setpts=PTS-STARTPTS ensures trimming at a good time, otherwise bad video issues
            filter += ` [v${i}];` // end
        });
