...
class FfmpegCommandService {
    ...
    createFfmpegCommand(command, allowUnknown = false) {
        this._validateCommandSchema(command, allowUnknown);
     ...
     * @private
     */
    _validateCommandSchema(command, allowUnknown) {
        const validationResult = Joi.validate(command, this._commandSchema, { allowUnknown });
    ...
    _buildInputs(command) {
        ...
        command.video.forEach(video => {
            inputs.push({
                ...
                input: Path.resolve(command.workingDirectory, video.filePath)
            });
        });
        ...
        if (command.audio) {
            command.audio.forEach(audio => {
                inputs.push({
                    ...
                    input: Path.resolve(command.workingDirectory, audio.filePath)
                });
            });
        }
    ...
    _buildFilterComplex(command) {
        ...
        if (command.textOverlay) {
            ...
            command.textOverlay.forEach((textOverlay,i) => {
                ...
                filter += `${i > 0 ? ',' : ''} drawtext=enable=1:text='${textOverlay.text.replace(bashCharsToEscapeRegex, "\\$&")}':x=${textOverlay.xLoc}:y=${textOverlay.yLoc}`;
