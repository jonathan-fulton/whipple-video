...
class FfmpegCommandService {
    ...
    _createCommandSchema(outputRequired = true) {
        ...
        const whippleCommandTextAlignment = Joi.object().keys({
            horizontal: Joi.any().only([null, 'left', 'center', 'right']),
            vertical: Joi.any().only([null, 'top', 'center', 'bottom']),
        }).optional();
        ...
        const whippleCommandTextOverlaySchema = Joi.object().keys({
            ...
            fadeIn: whippleCommandFadeEffectRequired,
            fadeOut: whippleCommandFadeEffectOptional,
            textAlignment: whippleCommandTextAlignment
        });
    ...
    _validateCommandSchema(command, schema, allowUnknown) {
        const validationResult = Joi.validate(command, schema, { allowUnknown });
    ...
    _buildFilterComplex(command) {
        ...
        if (command.textOverlay) {
            filter += ` [${lastVLabel}]`;
            filter += command.textOverlay.reduce(this._createFilterForTextOverlay.bind(this), '');
            filter += ` [v_text];`;
            lastVLabel = `v_text`;
        }
    ...
    _createFilterForTextOverlay(filter, textOverlay, i) {
        ...
        const textAlignment = textOverlay.textAlignment || null;

        const x = this._calculateHorizontalPosition(textAlignment ? textAlignment.horizontal : null, textOverlay.xLoc);
        const y = this._calculateVerticalPosition(textAlignment ? textAlignment.vertical : null, textOverlay.yLoc);

        const fontFile = Path.resolve(this._fontConfig.fontsDirectory, this._fontLookupObj[textOverlay.fontName].fontFilePath);
        filter += `${i > 0 ? ',' : ''} drawtext=enable=1:text='${textOverlay.text.replace(bashCharsToEscapeRegex, "\\$&")}':x=${x}:y=${y}`;
    ...
    _calculateHorizontalPosition(alignment, x) {
        switch (alignment) {
            default:
                return x;
            ...
            case 'right':
                return 'main_w-text_w';
        }
    }

    _calculateVerticalPosition(alignment, y) {
        switch (alignment) {
            default:
                return y;
